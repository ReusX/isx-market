/*
 * A note is copy, so it has two languages.
 *
 * `product_facts.note` carries the sentence that qualifies a fact — "balances
 * above 500m earn 5.00% at twelve months", "minimum facility-file fee, charged
 * on top of the rate". Those were written in English and rendered verbatim on
 * the Arabic page, which is the same leak `value_text_en` was added for.
 *
 * Same convention as the value columns: `note` is Arabic, `note_en` is our
 * English rendering. The view exposes both and the component picks by locale.
 */
alter table public.product_facts add column if not exists note_en text;
...
select
  ...
  f.source_url, f.source_excerpt, f.source_page, f.effective_date, f.verified_at,
  f.note, f.note_en,
...
/* A rebuilt view comes back with the owner's rights. Re-asserted every time
   these are recreated — this is the third migration that has had to. */
