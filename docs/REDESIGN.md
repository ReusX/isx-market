...
| 15 | `/login` `/signup` `/forgot-password` `/verify-email` `/auth/reset` `/profile` | NEW `site/AuthKit` (AuthShell · Field · PasswordField · AuthError · Submit · Outcome with the OLD prop API, on the site shell as a centred card) so `site/AuthScreens` and `site/ResetPasswordPage` are the production screens ported verbatim — the real Supabase calls and the Phase 0 fixes untouched. `site/ProfilePage` rebuilt as rows of facts with one action each (username edit, email not editable, password via emailed link, language = link, appearance, data counts, invite code, sign out). `styles/auth-page.css` (`ath-`) | old `auth/*`, `routes/Account`, `routes/ResetPassword`, `auth.css`, `profile.css` unused, delete in sweep | ☑ 2026-09-17 |
| 16 | `/about` `/contact` `/legal` `/privacy` | NEW `site/InfoPages` (`AboutPage` · `ContactPage` · `LegalPage`, one `Frame` at reading width with the sibling row at the foot) on `styles/info-page.css` (`inf-`). Copy untouched: dictionaries + `lib/legalContent` + `lib/infoData` | old `routes/AboutPage`/`ContactPage`, `info/LegalDoc`/`InfoChrome`, `info.css` unused, delete in sweep | ☑ 2026-09-17 |
| 17 | `/analysis`, `/analysis/[sym]` | NEW `site/AnalysisPage` (`AnalysisIndexPage` = one table of companies with search; `AnalysisPage` = the machine-written analysis as reading: headline, summary, verdict chip, key figures, bull/bear cases, themes, outlook, disclaimer). Still client-fetched from `/api/analysis/[sym]`, still ar-only | the two inline-styled pages, replaced | ☑ 2026-09-17 |
| 18 | system: 404 · 500 (`Toast`/`Overlay`/`Primitives` have no live callers outside old components) | NEW `site/StatePages` (`NotFoundPage`, `FaultPage`) on the site shell — one plain answer and real destinations; the `Error` object never rendered. Wired into both locales' `not-found.tsx`/`error.tsx` (the two-root-layout limitation on top-level 404s noted there still stands) | old `system/*`, `system.css` unused, delete in sweep | ☑ 2026-09-17 |
