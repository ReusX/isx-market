...
**Next:** `/charts` is to be REMOVED, not rebuilt — the user's call: «view full chart» on the homepage should expand `IndexChart` in place (fullscreen, the company chart's tools) rather than open a page. It has been a live URL, so it gets a 301 to `/`; `lightweight-charts` then leaves `package.json`. After that, row 10 `/banks`.
