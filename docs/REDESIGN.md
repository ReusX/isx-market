...
**Next:** `/c/[sym]` (row 4) — the company page, and the worst cost profile on the site: no `revalidate`, no `generateStaticParams`, so 104 companies × 2 locales are fully dynamic, and `CompanyDetail` then queries Supabase from the browser AND calls `/api/chart/[sym]`. Two function invocations and a direct query per view. Caching it is part of that row, not a separate job.
