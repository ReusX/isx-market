...
| 2 | `/` | `routes/Landing` (+ `home/Constellation`) | `landing.css` · old `HomePage`/`home.css` kept only until /market reuses its modules | ☑ 2026-09-16 |
...
- 2026-09-16 · Homepage: full-viewport blue opener with the ISX constellation (canvas: 4 macro anchors, 8 sector clusters of real tickers, one lit at a time) and the four doors — الأسواق · البنوك والتمويل · الاقتصاد العراقي · تعلّم. Renders bare (no sidebar). Pages that do not exist yet (deposits, loans, cards, CBI, budget) show «قريباً».
