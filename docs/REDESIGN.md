...
**`/charts` removed** (301 → `/`; «view full chart» expands `IndexChart` in place). **Row 10 `/banks` landed** — the banking door is open; its sub-pages (deposits, loans, cards) are «قريباً» rows in the rail, not routes. **Next: row 11** `/gold` `/fx` `/oil` (the economy door). Note the user considered marking الاقتصاد العراقي «قريباً» instead; confirm before building.
