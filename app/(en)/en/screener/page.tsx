...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ preset?: string }> }): Promise<Metadata> {
  ...
  const titles = messages('en').screener.page.titles
  if (!preset || !titles[preset] || preset === 'all') return { alternates: seoAlternates('/screener', 'en') }
  const path = `/screener?preset=${preset}`
  return { title: { absolute: titles[preset] }, alternates: seoAlternates(path, 'en'), openGraph: { url: absUrl(path, 'en'), title: titles[preset] } }
}
