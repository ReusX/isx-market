...
export async function generateMetadata({ searchParams }: Params): Promise<Metadata> {
  ...
  if (!session || session === sessions[0]) return { alternates: seoAlternates('/market') }
  const d = localeDate(session, 'ar')
  const path = `/market?date=${session}`
  return {
    title: { absolute: `اسعار الاسهم العراقية · جلسة ${d}` },
    description: `أسعار جميع شركات بورصة العراق في جلسة ${d}: آخر سعر، التغيّر، حجم التداول والقيمة السوقية لكل شركة مدرجة.`,
    alternates: seoAlternates(path),
    openGraph: { url: absUrl(path), title: `اسعار الاسهم العراقية · جلسة ${d}` },
  }
}
