...
import {
  fetchLive, fetchCompanyMeta, mergeCompanies, liveMcap, lastTradeNote, companyName,
    ...
    <main className="iq-page mk">
      ...
      <header className="mk-head">
        ...
        <dl className="mk-meta">
          <div>
            <dt>{m.session}</dt>
            <dd className="mk-meta-date">{session ? localeDate(session, locale) : '—'}</dd>
          </div>
          <div>
            <dt>{m.traded}</dt>
          ...
          <div>
            <dt>{m.companies}</dt>
            <dd><bdi>{activeCount}</bdi> <small>{m.of} <bdi>{listed.length}</bdi></small></dd>
          </div>
        </dl>
      </header>
      ...
      <section className="mk-summary" aria-label={m.summaryLabel}>
        <div className="mk-breadth">
          <span className="mk-cell-label">{m.breadth}</span>
          <div className="mk-breadth-bar" role="img"
            aria-label={ar
              ? m.breadthReading(String(counts.up), String(counts.flat), String(counts.down), String(counts.na), String(counts.traded))
          ...
          <div className="mk-breadth-keys">
            {([
              { id: 'up' as const, n: counts.up, label: m.up },
              { id: 'flat' as const, n: counts.flat, label: m.flat },
              { id: 'down' as const, n: counts.down, label: m.down },
              ...(counts.na > 0
                ? [{ id: 'na' as const, n: counts.na, label: m.noPrior }]
                : []),
            ]).map((k) => (
              <button key={k.id} type="button" className={k.id}
                aria-pressed={movement === k.id}
                onClick={() => setMovement((c) => (c === k.id ? 'all' : k.id))}>
                <bdi>{k.n}</bdi><span>{k.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="mk-metric">
          <span className="mk-cell-label">{m.tradedValue}</span>
          <strong><bdi>{compact.format(totals.value)}</bdi><small>IQD</small></strong>
        </div>

        <div className="mk-metric mk-metric-pair">
          <div>
            <span className="mk-cell-label">{m.volume}</span>
            <strong><bdi>{compact.format(totals.volume)}</bdi><small>{m.unitShares}</small></strong>
          </div>
          <div>
            <span className="mk-cell-label">{m.trades}</span>
            <strong><bdi>{nfInt.format(totals.trades)}</bdi></strong>
          </div>
        </div>

        <div className="mk-metric mk-most-active">
          <span className="mk-cell-label">{m.mostActive}</span>
          {totals.mostActive ? (
            <Link href={L(`/c/${totals.mostActive.sym}`)}>
              <strong>{companyName(totals.mostActive, totals.mostActive.sym, locale)}</strong>
              <span>
      ...
      <div className="mk-controls">
        <div className="mk-filter-tabs" role="tablist" aria-label={m.filterTabs}>
          {([
            { id: 'all' as const, label: m.tabAll },
            { id: 'gainers' as const, label: m.tabGainers },
            { id: 'losers' as const, label: m.tabLosers },
            { id: 'active' as const, label: m.tabActive },
          ]).map((t) => (
        ...
        <label className="mk-sector">
          <span className="sr-only">{m.sectorLabel}</span>
        ...
        <div className="mk-controls-end">
          ...
          <span className="mk-count"><bdi>{rows.length}</bdi> {m.countUnit}</span>
        </div>
      </div>

      {listing === 'suspended' ? (
        <p className="mk-note">
          {m.suspendedNote(String(STALE_DAYS))}
        </p>
      ) : null}
      ...
      {failed ? (
        <div className="mk-error" role="alert">
          ...
          <div>
            <strong>{m.loadFailedTitle}</strong>
            <p>{m.loadFailedNote}</p>
          </div>
          <button type="button" onClick={load}>{m.retry}</button>
        </div>
      ) : (
        <section className="mk-board" data-empty={showEmpty || undefined}
          aria-label={m.tableLabel}>
          <div className="mk-board-scroll">
            <table className="mk-table">
              <caption className="sr-only">
                {ar
                  ? m.caption(session ? localeDate(session, locale) : '', String(rows.length))
                  : `Iraq Stock Exchange movement for ${session} · ${rows.length} companies`}
              </caption>
              <thead>
                <tr>
                  <th className="mk-col-rank" scope="col">
                    <span className="sr-only">{m.rank}</span><span aria-hidden="true">#</span>
                  </th>
                  <th className="mk-col-watch" scope="col">
                    <span className="sr-only">{m.watch}</span>
                  </th>
                  <th className="mk-col-company" scope="col">{m.colCompany}</th>
                  <SortHeader k="price" label={m.colPrice} {...{ sortKey, sortDir, sortBy, m }} />
                  <SortHeader k="change" label={m.colChange} cls="mk-col-change-abs" {...{ sortKey, sortDir, sortBy, m }} />
                  <SortHeader k="pct" label={m.colPct} {...{ sortKey, sortDir, sortBy, m }} />
                  <SortHeader k="volume" label={m.colVolume} {...{ sortKey, sortDir, sortBy, m }} />
                  <SortHeader k="value" label={m.colValue} {...{ sortKey, sortDir, sortBy, m }} />
                  <SortHeader k="trades" label={m.colTrades} {...{ sortKey, sortDir, sortBy, m }} />
                  <SortHeader k="mcap" label={m.colMcap} {...{ sortKey, sortDir, sortBy, m }} />
                  <th className="mk-col-spark" scope="col">
                    <span className="sr-only">{m.colTrend}</span>
                  ...
                  return (
                    <tr key={c.sym} data-quiet={quiet || undefined}>
                      ...
                      <td className="mk-col-company">
                        <Link href={L(`/c/${c.sym}`)}>
                          ...
                          <span className="mk-identity">
                            <strong title={companyName(c, c.sym, locale)}>{companyName(c, c.sym, locale)}</strong>
                            <small>
                              <bdi className="mk-ticker">{c.sym}</bdi>
                              <em>{sectorShort(c.sec)}</em>
                              {quiet ? (
                                <span className="mk-flag" title={lastTradeNote(c, ar)}>
                                  {suspended ? m.rowSuspended : m.rowNoTrade}
                                </span>
                              ) : null}
                            </small>
                          </span>
                      ...
                      <td className="mk-col-num mk-price">
                        ...
                        {quiet && c.lastTrade && daysSinceTrade(c) > 5 ? <small>{localeDate(c.lastTrade, locale)}</small> : null}
                      </td>
                      ...
                      <td className="mk-col-num mk-col-change-abs">
                        {quiet || c.noPrior ? <Dash m={m} /> : <bdi className={abs.cls}>{abs.text}</bdi>}
                      </td>
                      <td className="mk-col-num">
                        {quiet || c.noPrior ? <Dash m={m} /> : <bdi className={`mk-pct ${pct.cls}`}>{pct.text}</bdi>}
                      </td>

                      <td className="mk-col-num">{quiet ? <Dash m={m} /> : <bdi>{nfInt.format(c.shares_traded ?? 0)}</bdi>}</td>
                      <td className="mk-col-num">{quiet ? <Dash m={m} /> : <bdi>{compact.format(c.vol ?? 0)}</bdi>}</td>
                      <td className="mk-col-num">{quiet ? <Dash m={m} /> : <bdi>{nfInt.format(c.deals ?? 0)}</bdi>}</td>
                      ...
                      <td className="mk-col-num">
                        {suspended || liveMcap(c) <= 0 ? <Dash m={m} /> : <bdi>{compact.format(liveMcap(c))}</bdi>}
                      </td>
          ...
          {showEmpty ? (
            <EmptyState m={m} query={query} tab={tab} sector={sector}
      ...
      <p className="mk-footnote">
        {m.footnote(session ? localeDate(session, locale) : '—')}
      </p>
    </main>
  )
}
...
function SortHeader({ k, label, cls = '', sortKey, sortDir, sortBy, m }: {
  k: SortKey; label: string; cls?: string; sortKey: SortKey
  sortDir: 'asc' | 'desc'; sortBy: (k: SortKey) => void; m: Messages['market']
}) {
  const active = sortKey === k
  const state = active
    ? sortDir === 'asc' ? m.sortedAsc : m.sortedDesc
    : m.notSorted
  return (
...
function Dash({ m }: { m: Messages['market'] }) {
  return <span className="mk-dash" aria-label={m.noValue}>—</span>
}
...
/* ── Empty ──────────────────────────────────────────────────────────────────
   ...
   hiding everything. */
function EmptyState({ m, query, tab, sector, movement, onlyWatchlist, sectorLabel, onReset }: {
  m: Messages['market']; query: string; tab: Tab; sector: string; movement: Movement
  onlyWatchlist: boolean; sectorLabel: (id: string) => string; onReset: () => void
}) {
  const e = m.empty
  const active: string[] = []
  if (tab !== 'all') active.push(e.tab[tab])
  if (movement !== 'all') active.push(e.move[movement])
  if (sector !== 'all') active.push(sectorLabel(sector))
  if (onlyWatchlist) active.push(e.watchlist)
  if (query.trim()) active.push(`${e.search}: ${query.trim()}`)
  ...
  return (
    <div className="mk-empty">
      ...
      <button type="button" onClick={onReset}>{e.reset}</button>
    </div>
  )
}
