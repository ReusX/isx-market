...
export const statistics: typeof ar = {
  ...
  page: {
    eyebrow: 'Markets',
    lede:    'Iraq Stock Exchange activity over time: how much trades, where, and who is buying. Official figures from the exchange\u2019s daily and monthly bulletins.',
    sub:     { overview: 'Overview', flow: 'Foreign flow', ownership: 'Ownership', holders: 'Major shareholders' },
    activity: 'Market activity',
    activityNote: (grain: string) => `One bar per ${grain} \u00b7 hover for the value`,
    grain:   { session: 'session', week: 'week', month: 'month' },
    total:   'Period total',
    perSession: 'Per session',
    best:    'Best session',
    sessions: 'Sessions',
    vsPrev:  (pct: string) => `${pct} vs the previous period`,
    noPrev:  'No previous period of the same length',
    sectors: 'Where the market trades',
    sectorsNote: (month: string) => `Each sector\u2019s share of traded value in ${month}. The monthly table is published after month end.`,
    ofValue: 'of traded value',
    foreign: 'Foreign investors',
    foreignNote: (n: string) => `Net foreign trading over the last ${n} sessions`,
    netBuy:  'Net buying',
    netSell: 'Net selling',
    ownership: 'Who owns the market',
    ownershipNote: (month: string) => `Iraqi and foreign share of deposited shares \u00b7 ${month}`,
    iraqis:  'Iraqis',
    ...
    about: {
      ...
      body: [
        'Traded value, volume and trades come from the exchange\u2019s daily bulletin, session by session since 5 March 2015 when the index was rebased. Monthly and weekly totals are sums of sessions; averages divide by actual sessions, not days.',
        'Sector shares come from the exchange\u2019s monthly report, published days after month end. Foreign flow comes from the non-Iraqis sheet of the daily bulletin. Ownership comes from the depository\u2019s monthly report.',
      ],
    },
  },
}
